...
static void test_full_drop_profile(void)
{
    const float speeds[] = {0.0f, 0.1f, 3.25f, 100.0f};
    const float sizes[] = {0.001f, 0.005f, 0.2f};
    liquid_particle_t p = {0};
    unsigned int i, j;
    int kind;
    reset_scene();
    ...
    for (kind = 0; kind < 3; kind++) {
        ...
        for (i = 0; i < sizeof(speeds) / sizeof(speeds[0]); i++) {
            p.velocity[0] = speeds[i];
            for (j = 0; j < sizeof(sizes) / sizeof(sizes[0]); j++) {
                float width, trail, tail, diameter;
                cfg.particle_size = sizes[j];
                liquid_droplet_shape(&p, 0, &width, &trail, &tail);
                diameter = 2 * cfg.particle_size * p.size_scale * width;
                assert(trail >= 0 && trail <= p.age);
                assert(speeds[i] * trail <= diameter * (p.secondary ? 1.6f : 2.4f) + 0.000001f);
                assert(tail >= 0.75f && tail <= 1.0f);
                liquid_droplet_shape(&p, 1, &width, &trail, &tail);
                close_to(trail, 0); close_to(tail, 1);
            }
        }
    }
    puts("PASS: drops retain bounded proportions across speed/size ranges; full tails; round endpoint");
}
