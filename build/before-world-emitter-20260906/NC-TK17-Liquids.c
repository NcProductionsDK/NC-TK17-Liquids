...
#define LIQUIDS_VERSION "0.7.71-native-audio-preset-selector"
                                    ...
                                    float position[3], float direction[3])
{
    ...
    if (!liquid_object_view_pivot(source_name, source_view) ||
        !liquid_object_view_pivot(target_name, target_view)) {
        /* Unnamespaced fallback used by some imported body scenes. */
        _snprintf(source_name, sizeof(source_name),
                  "Model01:penis_jointEnd");
        _snprintf(target_name, sizeof(target_name),
                  "Model01:penis_joint03");
        source_name[sizeof(source_name) - 1] = 0;
        target_name[sizeof(target_name) - 1] = 0;
        if (!liquid_object_view_pivot(source_name, source_view) ||
            !liquid_object_view_pivot(target_name, target_view)) return 0;
    }
    if (!liquid_view_to_world_point(source_view, source_world) ||
        !liquid_view_to_world_point(target_view, target_world)) return 0;
    position[0] = source_world[0];
    position[1] = source_world[1];
    position[2] = source_world[2];
    ...
    if (!liquid_vec3_normalize(direction)) {
        direction[0] = captured_camera_inverse[8];
        direction[1] = captured_camera_inverse[9];
        direction[2] = captured_camera_inverse[10];
